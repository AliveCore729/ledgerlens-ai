...
import { useTransactions, useUpdateCategory, useBulkUpdateCategory, exportTransactions, TransactionFilters } from '@/hooks/useTransactions';
...
export default function TransactionsPage() {
  ...
  const { mutate: bulkUpdateCategory, isPending: isBulkUpdating } = useBulkUpdateCategory();
  ...
  const handleBulkUpdate = (newCategory: string) => {
    if (selectedIds.length === 0) return;
    
    bulkUpdateCategory(
      { ids: selectedIds, category: newCategory },
      {
        onSuccess: () => {
          toast.success(`Successfully categorized ${selectedIds.length} transactions`);
          setSelectedIds([]);
        },
        onError: () => {
  ...
  const toggleSelectAll = (checked: boolean) => {
    if (!data?.data) return;
    if (checked) {
      setSelectedIds(data.data.map(tx => tx.id));
    } else {
      setSelectedIds([]);
    }
  };
    ...
    <div className="p-4 md:p-8 space-y-6 min-h-screen bg-background text-foreground">
      ...
      <div className="flex flex-col gap-4 p-4 bg-card rounded-lg border shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div className="flex items-center space-x-2 px-2 py-2 md:col-span-2 lg:col-span-1">
            <Switch
              id="needs-review"
              checked={filters.needsReview}
              onCheckedChange={(checked) => setFilters((f) => ({ ...f, needsReview: checked, page: 1 }))}
            />
            <Label htmlFor="needs-review" className="cursor-pointer">Needs Review</Label>
          </div>
        </div>
      ...
      {selectedIds.length > 0 && (
        <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-400 p-3 rounded-lg flex justify-between items-center animate-in fade-in slide-in-from-top-2">
          <span className="font-medium text-sm flex items-center gap-2">
            <Badge variant="outline" className="bg-emerald-500 text-white border-transparent">{selectedIds.length}</Badge>
            transactions selected
          </span>
          <div className="flex items-center gap-3">
            <span className="text-sm">Change category to:</span>
            ...
            <Button variant="ghost" size="sm" onClick={() => setSelectedIds([])} className="h-8 hover:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400">Cancel</Button>
          </div>
        </div>
      )}
                  ...
                  <TableRow key={tx.id} data-state={selectedIds.includes(tx.id) ? "selected" : undefined}>
                    <TableCell>
                      <Checkbox 
                        ...
                        onCheckedChange={(checked) => toggleSelectRow(tx.id, !!checked)}
                      />
                    </TableCell>
